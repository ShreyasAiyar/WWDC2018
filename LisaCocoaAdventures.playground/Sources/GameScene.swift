...
public class SceneTwo: SKScene,SKPhysicsContactDelegate {
  ...
  var reverseGravityLabel:SKLabelNode!
  ...
  func configureText(){
    ...
    let jumpLabel = OldSchoolText(fontSize: 20, fontText: "Jump")
    jumpLabel.position = CGPoint(x:-300, y: -380)
    heroCamera.addChild(jumpLabel)
    
    reverseGravityLabel = OldSchoolText(fontSize: 20, fontText: "Reverse Gravity")
    reverseGravityLabel.position = CGPoint(x:250, y:-380)
    heroCamera.addChild(reverseGravityLabel)
  ...
  func showObtainedFrameworkText(){
    ...
    let increaseAction = SKAction.scale(by: 2.3, duration: 0.3)
